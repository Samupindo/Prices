...
public class PurchaseSpecification {
  ...
  public static Specification<PurchaseModel> hasProductInShop(List<Integer> productInShop) {
    return (root, query, cb) -> {
      Join<PurchaseModel, PurchaseLineModel> purchaseLineJoin =
          root.join("purchaseLineModels", JoinType.INNER);
      Join<PurchaseLineModel, ProductInShopModel> productInShopJoin =
          purchaseLineJoin.join("productInShop", JoinType.INNER);
  ...
  public static Specification<PurchaseModel> hasPriceMax(BigDecimal priceMax) {
    return (root, query, criteriaBuilder) -> {
      ...
      subquery
          .select(criteriaBuilder.sum(pricesJoin.get("price")))
